...
enum FPGA_ACK_ERR { ERR_LEN = 1, ERR_NOACK, ERR_FUNCODE, ERR_CHECK };
...
int rcvRspFromFPGA(uint8_t* rcvBuf) {
	...
	for (i = 0; i < TRY_RCV_TIMES; i++) {
		iReadBytes = uart_read_bytes(UART_NUM_1, rcvBuf, BUF_SIZE,
					     WAIT_MSG_FROM_FPGA_MS / portTICK_RATE_MS);
		if (iReadBytes > 0) {
			// printf("rcvFromFpag: ");
...
int finigerprintTo20bytesResponse(uint8_t* au8Fingerprint, uint8_t* au8PufResp) {
	// uint8_t au8Fingerprint[199] = {0xf5, 00, 00, 00, 0xd, 0x6, 0x14, 0x40, 0x21, 0x1d, 0x9e, 0x98,
	// 0xc1, 0x3c, 0x19, 0x80, 0x41, 0x40, 0x12, 0xd5, 0xe1, 0x4b, 0x1e, 0x1, 0x21, 0x5, 0x18, 0xd8, 0x42,
	// 0xd, 0x21, 0x42, 0x62, 0x16, 0x90, 0x40, 0x2, 0x1a, 0x1b, 00, 0xc2, 0x25, 0x25, 0xc3, 0x2, 0x2c,
	// 0x87, 0x53, 0xe2, 0x2e, 0x1c, 0x81, 0xc2, 0x51, 0x1b, 0x2c, 0xa2, 00, 0xfe, 0xff, 0xff, 00, 00, 00,
	// 0x60, 00, 0x80, 00, 0x2, 00, 0x32, 00, 00, 00, 0x1, 00, 00, 00, 00, 00, 00, 00, 0x1, 00, 00, 00,
	// 00, 00, 00, 00, 00, 00, 00, 0xb2, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00,
	// 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00,
	// 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00,
	// 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00,
	// 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 0x2d, 0xf5};
	...
	for (i = 0; i < 5; i++) {
		...
		while ((iRet = getPufResponse(au8Challenge40bytes + i * 8, au8PufResponse4Bytes) != 0)
		       && (u8TryTimes > 0)) {
			rstFpga();
...
uint8_t* FindMajorElementIndex(uint8_t** data, int first_dim_len, int second_dim_len) {
...
int lblock_encrype(uint8_t* pu8PlainText, uint8_t* pu8Key, uint8_t* pu8EncrypedData,
		   uint8_t pu8LenbeforEncrype, uint8_t* pu8LenAfterEncrype) {
	/* 1. exam if pu8PlainText and pu8Key legal */
	if (pu8PlainText == NULL || pu8EncrypedData + pu8LenbeforEncrype - 1 == NULL || pu8Key == NULL
	    || pu8Key + 9 == NULL) {
		return -1;
	}
...
uint8_t au8ChallengeT[ 199 + 1 ] = {
	0xf5, 00,   00,	  00,	0xe,  0x2c, 0x27, 0xdb, 0x81, 0x30, 0x1e, 0x45, 0xe1, 0x36, 0x8f, 0xdc, 0x61,
	0x41, 0xa8, 0x9e, 0x21, 0x46, 0x16, 0xdf, 0x1,	0x4a, 0x9d, 0x9f, 0x61, 0x4f, 0x8a, 0x1f, 0x81, 0x10,
	0x94, 0x1c, 0x2,  0x27, 0x6,  0x9c, 0x62, 0x2d, 0x16, 0x1c, 0x2,  0x3d, 0x93, 0xc6, 0x22, 0x3d, 0xa1,
	0xdd, 0x42, 0x41, 0x89, 0x1e, 0x2,  0x4d, 0x93, 0xc8, 0x42, 00,	  00,	00,   0x60, 00,	  0x80, 00,
	0x2,  00,   0x32, 00,	00,   00,   0x1,  00,	00,   00,   00,	  00,	00,   00,   0x1,  00,	00,
	00,   00,   00,	  00,	00,   00,   00,	  00,	0xb2, 00,   00,	  00,	00,   00,   00,	  00,	00,
	00,   00,   00,	  00,	00,   00,   00,	  00,	00,   00,   00,	  00,	00,   00,   00,	  00,	00,
	00,   00,   00,	  00,	00,   00,   00,	  00,	00,   00,   00,	  00,	00,   00,   00,	  00,	00,
	00,   00,   00,	  00,	00,   00,   00,	  00,	00,   00,   00,	  00,	00,   00,   00,	  00,	00,
	00,   00,   00,	  00,	00,   00,   00,	  00,	00,   00,   00,	  00,	00,   00,   00,	  00,	00,
	00,   00,   00,	  00,	00,   00,   00,	  00,	00,   00,   00,	  00,	00,   00,   00,	  00,	00,
	00,   00,   00,	  00,	00,   00,   00,	  00,	00,   00,   0xb0, 0xf5
};
void fpga_test(void) {
	fpgaDriverInstall();
