...
//	fresh_first_100_samples();   //过滤前100个样本  ,  过滤了反而更不准
...
void max30102_read_reg_FIFO(uint8_t *red_1,uint8_t *red_2,uint8_t *red_3,uint8_t *ir_1,uint8_t *ir_2,uint8_t *ir_3){
...
void max30102_read_redLed_and_irLed(uint32_t *pun_red_led, uint32_t *pun_ir_led){
...
void calc_red_ir_led(uint8_t red_1,uint8_t red_2,uint8_t red_3,uint8_t ir_1,uint8_t ir_2,uint8_t ir_3,uint32_t *red_led,uint32_t *ir_led){
...
//			printf("red:%i ,ir:%i \r\n",red_led,ir_led);
			if( (red_led)<100000 || (ir_led)<100000){//如果读到的数据非法(手指还没放上去)，重新读
				illegal_cnt++;
			}
			else
				legalty = TRUE;
