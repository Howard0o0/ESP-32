...
static void taskRcvBleMsg(void)
{
        ...
        {
                /* waiting for ble msg */
                if (get_char_value(&bleRcvBuf, &bleRcvLen, 5) != 0)
                {
...
static void taskRegister(void)
{
        while (1)
        {
                ...
                while (!cjsonRegister && --try_times >= 0)
                        cjsonRegister = makeJsonForRegister();
